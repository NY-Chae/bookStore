...
class NetworkManager {    
    func fetchRequest (completion: @escaping(Result<BookModel,Error>) -> Void) { // 함수가 끝나면(decoding)실행되는 것이 escaping closure
        ...
        // URL로 값(형) 변경
     //   if let url = URL(string: urlString) {  // 지워도 실행은 되는 코드 하지만 나는 쓴다.
            
            // (request 만들기 전에) 쿼리가 필요해 쿼리 생성: URL component
            var urlComponent = URLComponents(string: urlString)
            // 쿼리 등록
            urlComponent?.queryItems = [URLQueryItem(name: "query", value: "미움받을 용기")]
            ...
            task.resume() // resume 없으면 실행 안된다!! ㄱ 해..
        }
